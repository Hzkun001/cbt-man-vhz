    ...
    <div className="w-full space-y-8 animate-in fade-in duration-500 pb-16">
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        <KpiCard
          ...
          subtitle={`${activeExams.length} Aktif ΓÇó ${upcomingExamCount} Mendatang`}
                  ...
                  <div key={exam.id} className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200/60 dark:border-slate-700/60">
                    ...
                    <div className="flex items-center gap-2 text-[11px] text-slate-500">
                      ...
                      <span suppressHydrationWarning>
                        {new Date(exam.beginAt).toLocaleDateString("id-ID", { day: "numeric", month: "short", timeZone: "Asia/Jakarta" })} ΓÇó {new Date(exam.beginAt).toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit", timeZone: "Asia/Jakarta" })}
                      </span>
                    </div>
                  </div>
                ))}
