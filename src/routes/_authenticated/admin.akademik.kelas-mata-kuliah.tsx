    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        {items.map((item) => {
          ...
          return <Card key={item.id} className="p-4"><div className="flex items-start justify-between"><div><h2 className="font-semibold">{mk?.nama ?? "Mata kuliah tidak ditemukan"}</h2><p className="text-sm text-muted-foreground">{smt?.nama ?? "Tanpa semester"} · Kelas {item.kodeKelas || "-"}</p><p className="mt-2 text-xs text-muted-foreground">{item.pesertaIds.length} peserta · {item.pengampuIds.length} pengampu</p></div><div className="flex gap-1"><Button variant="ghost" size="icon" onClick={() => edit(item)} aria-label="Edit kelas mata kuliah" title="Edit kelas mata kuliah"><Pencil className="h-4 w-4" /></Button><Button variant="ghost" size="icon" onClick={() => remove(item.id)} aria-label="Hapus kelas mata kuliah" title="Hapus kelas mata kuliah"><Trash2 className="h-4 w-4 text-destructive" /></Button></div></div></Card>;
        })}
